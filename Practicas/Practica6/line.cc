...
int main() {
  std::cout << "#" << std::setw(GAP - 1) << "line (B)" << std::setw(GAP)
            << "time (Âµs)" << std::endl;
  ...
  for (unsigned line = 1; line <= MAXLINE; line <<= 1)  // line in bytes
  {
    ...
    std::nth_element(score.begin(), score.begin() + score.size() / 2,
                     score.end());

    std::cout << std::setw(GAP) << line << std::setw(GAP) << std::fixed
              << std::setprecision(1) << std::setw(GAP)
              << score[score.size() / 2].count() << std::endl;
  }
}
